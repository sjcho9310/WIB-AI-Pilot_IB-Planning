...
# 조회 범위 라벨 헬퍼
# ---------------------------------------------------------------------------
def get_scope_label(ss: dict) -> str:
    view = ss.get("view_level")
    if view == "all":
        return "IB부문 통합"
    if view == "division":
        return f"{ss.get('selected_division', '')} 본부"
    if view == "department":
        return ss.get("selected_department", "")
    return "IB부문 전체"
...
def update_deals(edited_df: pd.DataFrame, ss_state: dict):
    ...
    # deal_id + 트랜치번호 기준으로 편집된 행 매핑 (삭제 대상 제외)
    edited_map = {
        (str(r["deal_id"]), str(r["트랜치번호"])): r[save_cols]
        for _, r in edited_df.iterrows()
        if not r.get("삭제", False)
    }
